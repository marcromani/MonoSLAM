...
int main(int argc, char *argv[]) {
    ...
    tol = 2; // Default sample reprojection error tolerance

    if (!getOptions(argc, argv, rows, cols, squareSize, tol)) {
    ...
    cout << "Distortion coeficients: " << distCoeffs << endl;
}
...
void printUsage(const char *execName) {
    ...
    cerr << "Usage: " << execName << " rows cols square_size [tol] (-h, --help for help)" << endl;
}
